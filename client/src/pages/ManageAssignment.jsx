...
const ManageAssignments = () => {
  ...
  // Function to convert Dropbox sharing link to direct download link
  const getDirectDownloadLink = (dropboxUrl) => {
    if (!dropboxUrl) return null;
    
    // Handle different Dropbox URL formats
    if (dropboxUrl.includes('dropbox.com')) {
      // Check if it's already a dl.dropboxusercontent.com URL
      if (dropboxUrl.includes('dl.dropboxusercontent.com')) {
        return dropboxUrl.includes('?dl=1') ? dropboxUrl : `${dropboxUrl}${dropboxUrl.includes('?') ? '&' : '?'}dl=1`;
      }
  ...
  const downloadAllSubmissions = async () => {
    ...
    try {
      // Filter submissions to only those with fileUrls
      const submissionsToDownload = submissions.filter(
        submission => submission.fileUrl && submission.status !== 'Pending'
      );
      
      if (submissionsToDownload.length === 0) {
        toast.error("No submitted files available to download");
        setDownloadingAll(false);
        return;
      }
      
      toast.success(`Starting download of ${submissionsToDownload.length} files...`);
      ...
      for (const submission of submissionsToDownload) {
        try {
          ...
          const directUrl = getDirectDownloadLink(submission.fileUrl);
          if (!directUrl) {
            errorCount++;
            continue;
          }
          
          // Create filename with student name for better organization
          const studentName = submission.userId?.name || 'Unknown_Student';
          const safeStudentName = studentName.replace(/[^a-z0-9]/gi, '_').toLowerCase();
          
          // Get file extension from URL or default to pdf
          let fileExtension = directUrl.split('.').pop().split('?')[0];
          if (!fileExtension || fileExtension.length > 5) {
            fileExtension = 'pdf';
          }
          
          // Create meaningful filename
          const assignmentTitle = selectedAssignment.title || 'assignment';
          const safeAssignmentTitle = assignmentTitle.replace(/[^a-z0-9]/gi, '_').toLowerCase();
          const filename = `${safeStudentName}_${safeAssignmentTitle}.${fileExtension}`;
          ...
          console.error(`Error downloading file for ${submission.userId?.name}:`, error);
          errorCount++;
        }
      }
      
      if (successCount > 0) {
        toast.success(`Downloaded ${successCount} submission${successCount !== 1 ? 's' : ''} successfully`);
      }
      if (errorCount > 0) {
        toast.error(`Failed to download ${errorCount} file${errorCount !== 1 ? 's' : ''}`);
      }
    } catch (error) {
      toast.error("Error downloading submissions");
                        ...
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
                          ...
                          <div className="flex space-x-2 mt-2 sm:mt-0">
                            <button
                              ...
                              disabled={downloadingAll || submissions.filter(s => s.fileUrl && s.status !== 'Pending').length === 0}
                              className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50"
                            >
                              <Download className="mr-2 h-4 w-4" />
                              {downloadingAll ? 'Downloading...' : 'Download All Submissions'}
                            </button>
                                ...
                                <tr key={submission._id}>
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div className="flex space-x-2">
                                      <a 
                                        href={getDirectDownloadLink(submission.fileUrl)} 
                                        target="_blank" 
